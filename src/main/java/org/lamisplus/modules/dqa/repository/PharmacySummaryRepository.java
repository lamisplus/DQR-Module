...
public interface PharmacySummaryRepository extends JpaRepository<DQA, Long> {
    ...
    List<PatientSummaryDTOProjection> getPharmacyLastRefillDurationSumm(Long facilityId);


    @Query(value = "SELECT count(regimen) AS numerator, count(hospitalNumber) as denominator, ROUND((CAST(count(regimen) AS DECIMAL) / count(hospitalNumber)) * 100, 2) AS performance FROM (\n" +
            "SELECT e.unique_id AS patientId ,p.hospital_number AS hospitalNumber, INITCAP(p.sex) AS sex\n" +
            ...
            "  ORDER BY p.id DESC) dd", nativeQuery = true)
    List<PatientSummaryDTOProjection> getPharmacyRegimenSumm(Long facilityId);
